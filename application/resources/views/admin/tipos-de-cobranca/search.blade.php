                                ...
                                @if (count($resultados) > 0)
                                    @foreach ($resultados as $resultado)
                                        <tr>
                                            ...
                                            <td class="border-bottom-0">
                                                <a href="{{ url('admin/tipo-de-cobranca/' . $resultado->id) }}"
                                                    class="m-1 btn btn-primary" title="Detalhar">
                                                    <i class="ti ti-search"></i>
                                                </a>

                                                <a href="{{ url('admin/tipo-de-cobranca/' . $resultado->id . '/edit') }}"
                                                    class="m-1 btn btn-success" title="Editar">
                                                    <i class="ti ti-edit"></i>
                                                </a>

                                                @if ($resultado->status === 'ativo')
                                                    <a href="{{ url('admin/tipo-de-cobranca/inativar/' . $resultado->id) }}"
                                                        class="m-1 btn btn-danger" title="Inativar">
                                                        <i class="ti ti-lock"></i>
                                                    </a>
                                                @elseif ($resultado->status === 'inativo')
                                                    <a href="{{ url('admin/tipo-de-cobranca/reativar/' . $resultado->id) }}"
